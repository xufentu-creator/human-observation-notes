...
for(const o of observations){
 ...
 const page=shell({title:`${o.title} — ${SITE}`,description:o.desc,canonical:`${BASE}/observations/${o.slug}/`,jsonld:`<script type="application/ld+json">${JSON.stringify(json)}</script>`,body:`<main><article class="article"><div class="eyebrow">Public Observation Record</div><h1>${esc(o.title)}</h1><div class="meta"><span>Published ${esc(o.date||'Undated')}</span><span>Updated ${esc(o.updated||o.date||'')}</span><span>${o.minutes} min read</span><span>Author ${AUTHOR}</span></div><div class="topics">${o.topics.map(t=>`<span>${esc(t)}</span>`).join('')}</div><div class="content">${o.html}</div><section class="share-panel" aria-label="Share this observation"><h2>Share this observation</h2><p>Share the canonical reading link after finishing this record.</p><div class="actions"><button class="btn" onclick="shareObservation()">Share</button><a class="btn" href="https://twitter.com/intent/tweet?text=${encodeURIComponent(o.title)}&url=${encodeURIComponent(`${BASE}/observations/${o.slug}/`)}" target="_blank" rel="noopener">Share on X</a><a class="btn" href="https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`${BASE}/observations/${o.slug}/`)}" target="_blank" rel="noopener">Share on LinkedIn</a><button class="btn" onclick="navigator.clipboard.writeText(location.href).then(()=>this.textContent='Link copied')">Copy link</button></div></section><section class="evidence"><h2>Record and citation</h2><dl><dt>Canonical URL</dt><dd><a href="${BASE}/observations/${o.slug}/">${BASE}/observations/${o.slug}/</a></dd><dt>Original source</dt><dd><a href="${source}">${esc(o.rel)}</a></dd><dt>Version history</dt><dd><a href="${history}">GitHub commit history</a></dd><dt>Immutable version</dt><dd><a href="${immutable}">${o.commit?o.commit.slice(0,12):'Current main version'}</a></dd><dt>SHA-256</dt><dd>${o.hash}</dd><dt>Citation</dt><dd id="citation">${esc(cite)}</dd></dl><div class="actions"><button class="btn" onclick="navigator.clipboard.writeText(document.getElementById('citation').textContent)">Copy citation</button><a class="btn" href="${source}">View source</a><a class="btn" href="${history}">View history</a><button class="btn" onclick="window.print()">Print / Save PDF</button></div></section><div class="footer"><a href="${BASE}/">All observations</a> · <a href="${BASE}/archive/">Archive</a></div></article></main><script>async function shareObservation(){const data={title:${JSON.stringify(o.title)},text:${JSON.stringify(o.desc)},url:location.href};if(navigator.share){try{await navigator.share(data);return}catch(e){if(e&&e.name==='AbortError')return}}await navigator.clipboard.writeText(location.href);alert('Reading link copied.')}</script>`});
